 ...
 *
 * ⚠ NO LONGER USED as of the Supabase migration - this seeded the old
 * backend/data/db.json JSON-file store, which no route reads from
 * anymore (see lib/supabase.js). The equivalent one-time data load into
 * Supabase is supabase/seed/002_migrate_existing_data.sql, run from the
 * Supabase SQL Editor. Left in place only as a record of the
 * pre-migration defaults; running it has no effect on the live site.
 */
