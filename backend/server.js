...
// ---- Required configuration check (fail loudly at startup, not silently
// at request time) - this MUST run before any route file is required
// below: routes/*.js all pull in lib/supabase.js at the top of the file,
// and that module throws immediately if the Supabase env vars are
// missing. Checking here first turns that into one clear message instead
// of a raw stack trace from deep inside a require() chain.
const REQUIRED_ENV = ["ADMIN_EMAIL", "ADMIN_PASSWORD_HASH", "SESSION_SECRET", "SUPABASE_URL", "SUPABASE_SECRET_KEY"];
...
if (missingEnv.length) {
  ...
  console.error(`SUPABASE_URL and SUPABASE_SECRET_KEY come from your Supabase project's Settings -> API page.\n`);
  process.exit(1);
}
